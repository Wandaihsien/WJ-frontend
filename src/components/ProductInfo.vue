...
<template>
  <div
    @click="$emit('close')"
    class="fixed inset-0 bg-black bg-opacity-50 z-[996]"
  ></div>

  <div
    class="w-[calc(100%-40px)] h-[480px] ml-[20px] mr-[20px] mt-[50px] overflow-y-auto fixed top-[150px] flex flex-col bg-white z-[997] sm:max-w-[640px] sm:max-h-[388px] sm:flex-row sm:mx-auto sm:left-1/2 sm:translate-x-[-50%]"
  >
    <div
      @click="$emit('close')"
      class="fixed right-[20px] top-[200px] w-[36px] h-[36px] cursor-pointer bg-black text-white flex justify-center items-center sm:top-[0] sm:right-[0]"
    >
      <Cross />
    </div>
    <div class="w-full">
    ...
    <div class="w-full">
      ...
      <div class="w-full mt-[40px] mb-[10px]">
        <button
          @click="addProductToCart(product, addQuantity)"
